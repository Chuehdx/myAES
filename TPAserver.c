...
int main(void)
{
	int server_socket, storage_socket, client_socket , c , read_size , loop = 1 , authenticated = 0;
	...
	while(loop){//
		...
		while(!authenticated){//check if the user is authenticated or not
			...
			if((read_size = recv(client_socket , client_message , sizeof(client_message) , 0)) > 0 ){
				...
				if(myTPA_authentication(user_name,password,token)){
					...
					authenticated = 1;//user is authenticated
