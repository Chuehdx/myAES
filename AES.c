...
int old_decrypt(unsigned char *inbuff, unsigned char *outbuf,AES_KEY key,unsigned char *iv, int size){
...
int old_encrypt(unsigned char *inbuff, unsigned char *outbuf,AES_KEY key,unsigned char *iv, int size){
	int olen=0, tlen=0;
	/*EVP_CIPHER_CTX  ctx;
	EVP_CIPHER_CTX_init(&ctx);
...
int new_decrypt(unsigned char *inbuff, unsigned char *outbuf,char* key,unsigned char *iv, int size){
	int olen=0, tlen=0, n=0;
	EVP_CIPHER_CTX  ctx;
	EVP_CIPHER_CTX_init(&ctx);
	EVP_DecryptInit_ex(&ctx, EVP_aes_256_cbc(),NULL, key, iv);
...
int new_encrypt(unsigned char *inbuff, unsigned char *outbuf,char* key,unsigned char *iv, int size){
	int olen=0, tlen=0;
	EVP_CIPHER_CTX  ctx;
	EVP_CIPHER_CTX_init(&ctx);
	EVP_EncryptInit_ex(&ctx, EVP_aes_256_cbc(),NULL, key, iv);
...
main(void){
	...
	//for(int i=0;i<10;i++){
		...
		/*AES_set_encrypt_key(key, 256, &enc_key);
    		old_encrypt(plain, entext,enc_key,iv,file_size);
		AES_set_decrypt_key(key, 256, &dec_key);	
		memset(iv, 0x00, AES_BLOCK_SIZE);	
		old_decrypt(entext,detext,dec_key,iv,en_size);*/


		//printf("Decrypted text:\n%s\n",detext);
		en_size = new_encrypt(plain, entext,key,iv,file_size);
		if(en_size)
			printf("");
		...
		if(new_decrypt(entext,detext,key,iv,en_size))
			printf("");
			//printf("Decrypted text:\n%s\n",detext);
		else 
			puts("de fail");
